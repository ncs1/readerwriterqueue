...
public:
  ...
  bool try_dequeue(U &result) AE_NO_TSAN
  {
    ...
    non_empty_front_block:
      ...
      fence(memory_order_release); // Expose possibly pending changes to frontBlock->front from last
                                   // dequeue
...
private:
  ...
  struct ReentrantGuard
  {
    ...
    {
      assert(!inSection && "Concurrent (or re-entrant) enqueue or dequeue operation detected (only "
                           "one thread at a time may hold the producer or consumer role)");
      inSection = true;
    }
  ...
  struct Block
  {
    ...
    char cachelineFiller1[MOODYCAMEL_CACHE_LINE_SIZE - sizeof(weak_atomic<size_t>) -
                          sizeof(size_t)]; // next isn't very contended, but we don't want it on the
                                           // same cache line as tail (which is)
...
public:
  ...
  // returned instead.
  // Must be called only from the consumer thread.
  AE_FORCEINLINE T *peek() const AE_NO_TSAN { return inner.peek(); }
  ...
  //       the case where the producer was writing to the same block the consumer was
  //       reading from the whole time.
  AE_FORCEINLINE size_t max_capacity() const { return inner.max_capacity(); }
